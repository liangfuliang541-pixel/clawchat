...
async function notifyGroup(groupId: string, message: string) {
  ...
  // 并发发送，而不是顺序发送
  await Promise.all(members.map((member) => notificationService.send(member.userId, message)));
}

// ✅ 使用 Promise.allSettled 处理部分失败
async function batchDeleteMessages(messageIds: string[]) {
  const results = await Promise.allSettled(messageIds.map((id) => messageRepository.delete(id)));
...
async function warmupCache() {
  ...
  await Promise.all(hotUsers.map((user) => cacheService.set(`user:${user.id}`, user, 3600)));
