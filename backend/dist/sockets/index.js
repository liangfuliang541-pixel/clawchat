...
async function maybeTriggerHermesAgent(io, conversationId, senderUserId) {
    ...
    for (const agentConfig of agents) {
        if (!agentConfig.enabled)
            continue;
        const shouldTrigger = latestContent.includes(`@${agentConfig.name.toLowerCase()}`) ||
        ...
        const agentUserId = (agentUser._id.toString?.() || agentUser._id);
        ...
        const content = await hermesBridgeService.invoke(agentConfig._id || agentConfig.name, conversationId, chatHistory);
        if (!content) {
        ...
        logger.info({ messageId: reply._id, agent: agentConfig.name, conversationId }, 'Hermes Agent reply broadcasted');
    }
}
